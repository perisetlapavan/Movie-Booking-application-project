...
class MovieFilter extends React.Component {
    ...
    state = {
      ...
      handleInputChange = () => {
          ...
          console.log(moviesData[0].artists[1].id)
          var copyData = moviesData.filter(   (item)=>{if(this.state.MovieName===''){return  (item.genres.includes(this.state.Genre))}
                                                        else{return  (item.genres.includes(this.state.Genre) && item.title.includes(this.state.MovieName)  )}})
                            ...
                            <FormControl fullWidth variant="standard" >
                                ...
                                >
                                    {this.state.artists.map((item) => (
                                        <MenuItem value={item.id} key={item.id}>{item.first_name}  {item.last_name}</MenuItem>
                                    ))}
